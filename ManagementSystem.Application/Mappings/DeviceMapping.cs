...
public static class DeviceMappings
{
    ...
    public static Device MapToDevice(this DeviceRequest request)
    {
        return new Device
        {
            ...
            DeviceSerialNo = string.IsNullOrEmpty(request.SerialNo) ? request.SerialNo : request.SerialNo.Trim(),
            DeviceIMEINo = string.IsNullOrEmpty(request.IMEINo) ? request.IMEINo : request.IMEINo.Trim(),
    ...
    public static Device MapToDevice(this DeviceRequest request, Device device)
    {
        return new Device
        {
            ...
            DepartmentId = request.DepartId,
            DeviceTypeId = request.TypeId,
            DeviceSerialNo = string.IsNullOrEmpty(request.SerialNo) ? device.DeviceSerialNo : request.SerialNo.Trim(),
            DeviceIMEINo = string.IsNullOrEmpty(request.IMEINo) ? device.DeviceIMEINo: request.IMEINo.Trim(),
            Condition = request.Condition,
            PurchasedDate = request.PurchasedDate,
            PurchasedPrice = request.PurchasedPrice,
            Year = request.PurchasedDate.Year,
